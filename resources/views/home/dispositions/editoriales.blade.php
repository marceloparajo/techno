...
@php
	$itemsBySlices = 4;
	$slices = (int)ceil(count($news)/$itemsBySlices);
	$currentSlice = 0;
@endphp
	...
	<div class="container columnistas-home">
		<div class="d-flex flex-column flex-md-row align-items-start">
			<div class="row w-100 flex-md-grow-1">
				...
				<h5 class="mx-2 mx-md-0">{{ __("editorials") }}</h5>

				<div id="carouselEditoriales" class="carousel slide" data-ride="carousel">
					<ol class="carousel-indicators">
						@for ($i = 0; $i < $slices; $i++)
							<li data-target="#carouselEditoriales" data-slide-to="{{ $i }}" class="@php if($i == 0) {echo 'active';} @endphp"></li>
						@endfor
					</ol>
						...
						<div class="carousel-item @php if($i == 0) { echo 'active';} @endphp">
							<div class="row d-flex">
								@foreach(array_slice($news, ($i*$itemsBySlices), $itemsBySlices) as $key => $new)
									<article class="col-12 col-sm-6 col-lg-3 columna editores d-flex px-2">
										<a href="{{ $new['permalink'] }}" class=" align-items-stretch">
											...
											@endif --}}
											<figure>
												{!! $imageHelper->getLazyImages( $new['main_image']['srcs']['small-wide'], 270, $new['main_image']['caption'],'img-fluid','270x152') !!}
											</figure>
